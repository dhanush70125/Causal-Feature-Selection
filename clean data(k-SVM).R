...
#fitting k-SVM on traing_set
#install.packages("e1071")
library(e1071)
ksvm_classifier = svm(formula = RuptureState ~.,
                      data = train_set,
                      type = 'C-classification',
                      kernel = 'radial')

#predicting test set results
y_pred = predict(ksvm_classifier, newdata = test_set[-1])
...
cv = lapply(folds, function(x) {
  ...
  ksvm_classifier = svm(formula = RuptureState ~.,
                        ...
                        type = 'C-classification',
                        kernel = 'radial')
  y_pred = predict(ksvm_classifier, newdata = test_fold[-1])
