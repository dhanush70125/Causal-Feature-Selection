...
#fitting Naive Bayes on train_set
#install.packages('e1071')
library(e1071)
naivebayes = naiveBayes(x = train_set[-1],
                        y = train_set$RuptureState)

#preidcting the test_set results
y_pred = predict(naivebayes, newdata = test_set[-1])
...
cv = lapply(folds, function(z) {
  ...
  naivebayes = naiveBayes(x = train_fold[-1],
                          y = train_fold$RuptureState)
  y_pred = predict(naivebayes, newdata = test_fold[-1])
