...
#fitting KNN on train_set and also predicting the test_set results
library(class)
y_pred = knn(train = train_set[, -1],
             test = test_set[, -1],
             cl = train_set[, 1],
             k = 5)
...
# install.packages('caret')
library(caret)
grid_tree = train(form = RuptureState ~ ., 
                  data = train_set, method = 'knn')

tuned_parameters = grid_tree$bestTune

#fitting KNN on train_set including tuned parameters and also predicting the test_set results
library(class)
y_pred_tuned = knn(train = train_set[, -1],
                   test = test_set[, -1],
                   cl = train_set[, 1],
                   k = 9)

#making confusion matrix
cm_tuned = table(test_set[,1], y_pred_tuned)